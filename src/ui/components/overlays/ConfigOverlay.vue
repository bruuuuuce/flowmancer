<template>
  ...
  >
    <div class="overlay-header">
      <h3 style="margin: 0;">Config JSON</h3>
    ...
    <textarea :value="modelValue" @input="$emit('update:modelValue', ($event.target as HTMLTextAreaElement).value)" rows="12"></textarea>
    <div style="margin-top:8px; display:flex; gap:8px;">
      <button class="btn" @click="$emit('apply')">Apply</button>
      <button class="btn" @click="$emit('load-baseline')">Load baseline</button>
    </div>
    <div style="color:#f88" v-if="cfgErr">{{ cfgErr }}</div>
  </div>
</template>
